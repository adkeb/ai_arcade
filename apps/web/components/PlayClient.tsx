...
const iframeFeaturePolicy =
  "accelerometer 'none'; autoplay 'none'; camera 'none'; clipboard-read 'none'; clipboard-write 'none'; encrypted-media 'none'; fullscreen 'none'; geolocation 'none'; gyroscope 'none'; microphone 'none'; midi 'none'; payment 'none'; usb 'none'; web-share 'none'; xr-spatial-tracking 'none'";
...
export function PlayClient({ gameId }: { gameId: string }) {
    ...
    async function load() {
      ...
      try {
        const [gameData, manifestData] = await Promise.all([
          ...
          readApi<ManifestPayload>(
            await fetch(`/api/games/${gameId}/manifest`),
          ),
        ]);
        if (cancelled) return;
        ...
        const remoteManifest = (await fetch(manifestData.manifestUrl).then(
          (response) => {
            if (!response.ok)
        ...
        const entry = remoteManifest.files.find(
          (file) => file.path === remoteManifest.entry,
        );
        if (!entry)
          throw new Error(
            "Manifest does not contain an index.html entry file.",
          );
        setEntryUrl(entry.url);
      } catch (err) {
        const message =
          err instanceof Error ? err.message : "Unable to load game.";
        setStatus("failed");
        setError(message);
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (!data || typeof data !== "object" || data.source !== "ai-arcade-game")
        return;
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-slate-950">
            {game?.title ?? "正在加载游戏文件或初始化运行环境"}
          </h1>
          <p className="mt-2 max-w-3xl text-slate-600">
            {game?.description ??
              "Fetching game meta, manifest, and remote entry file."}
          </p>
        ...
        <div className="flex gap-2">
          <Link
            className="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold"
            href="/"
          >
            <Home size={16} /> Home
          </Link>
        ...
        <aside className="space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              The iframe loads the remote entry from the object-storage
              manifest.
            </p>
          </div>
