...
import { buildManifest, objectKeyForArtifact } from "@ai-arcade/storage/artifact-protocol";
import {
  contentTypeForPath,
  getStorageConfig,
  publicObjectUrl,
...
import type { GameDesignSpec, GameSourceFiles, PackagedArtifact } from "../types";
...
export async function runBuildPackagerAgent(params: {
  ...
  return {
    ...
    coverUrl: publicObjectUrl(bucket, objectKeyForArtifact(gameId, version, "cover.svg"), config),
    bucket,
    prefix,
    artifactBaseUrl: publicPrefixUrl(bucket, prefix, config),
    manifestUrl: publicObjectUrl(bucket, objectKeyForArtifact(gameId, version, "manifest.json"), config),
    manifest,
    files
  };
}
